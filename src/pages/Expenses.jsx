...
export default function Expenses() {
  ...
  const [filters, setFilters] = useState({ categoryId: "", startDate: "", endDate: "", paymentMethod: "" });
  ...
  const applyFilter = async () => {
    ...
    try {
      ...
      setTotalPages(0); setTotalElements(Array.isArray(data) ? data.length : 0);
      setActiveFilters({ ...filters });
      setIsFiltered(true);
      setShowFilter(false);
    } catch (e) {
      console.error(e); setPageError("Failed to filter expenses.");
    } finally { setLoading(false); }
  };

  const clearFilters = () => {
    setFilters({ categoryId: "", startDate: "", endDate: "", paymentMethod: "" });
    setActiveFilters({});
    setIsFiltered(false);
    setPage(0);
    loadExpenses();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.amount || isNaN(formData.amount)) { setFormError("Valid amount is required."); return; }
    if (!formData.expenseDate) { setFormError("Date is required."); return; }
    setSubmitting(true);
    try {
      await createExpense({
        ...formData,
  ...
  const closeModal = () => {
    setShowModal(false);
    setFormData({ title: "", amount: "", expenseDate: "", categoryId: "", paymentMethod: "", notes: "" });
    setFormError("");
  };
  ...
  return (
    <MainLayout>
      ...
      <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", padding: "36px 40px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
              <div style={{ width: 40, height: 40, background: "#EEF2FF", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                  <path d="M3 5h14M3 10h14M3 15h8" stroke="#6366F1" strokeWidth="1.8" strokeLinecap="round"/>
                  <circle cx="16" cy="15" r="3" stroke="#6366F1" strokeWidth="1.6"/>
                </svg>
              </div>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: "#0F172A", margin: 0, letterSpacing: "-0.5px" }}>Expenses</h1>
            </div>
            <p style={{ fontSize: 14, color: "#64748B", margin: "0 0 0 52px" }}>
              {loading ? "Loading…" : `${totalElements || expenses.length} total expenses`}
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, background: activeFilterCount > 0 ? "#EEF2FF" : "#F8FAFC", color: activeFilterCount > 0 ? "#4338CA" : "#374151", border: `1.5px solid ${activeFilterCount > 0 ? "#C7D2FE" : "#E2E8F0"}`, borderRadius: 10, padding: "10px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}
            >
              ...
              Filters
              {activeFilterCount > 0 && (
                <span style={{ background: "#6366F1", color: "#fff", borderRadius: "50%", width: 18, height: 18, fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {activeFilterCount}
                </span>
              )}
            </button>
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, background: "#6366F1", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 2px 8px rgba(99,102,241,0.3)" }}
        ...
        {/* ── Filter Panel ── */}
        {showFilter && (
          <div style={{ background: "#fff", border: "1.5px solid #E2E8F0", borderRadius: 14, padding: "20px 24px", marginBottom: 20, animation: "fadeIn 0.2s ease", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14, marginBottom: 16 }}>
              {/* Category */}
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#64748B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>Category</label>
              ...
              {/* Payment Method */}
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#64748B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>Payment Method</label>
              ...
              {/* Start Date */}
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#64748B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>From Date</label>
              ...
              {/* End Date */}
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#64748B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>To Date</label>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button onClick={clearFilters} style={{ padding: "9px 18px", background: "#F8FAFC", color: "#374151", border: "1.5px solid #E2E8F0", borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                Clear
              </button>
              <button onClick={applyFilter} style={{ padding: "9px 18px", background: "#6366F1", color: "#fff", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                Apply Filters
              </button>
            </div>
          </div>
        ...
        {!loading && expenses.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 20 }}>
            {[
              { label: "Total Expenses", value: `₹${formatAmount(totalAmount)}`, icon: "💸", bg: "#EEF2FF", color: "#4338CA" },
              { label: "No. of Records", value: expenses.length, icon: "📋", bg: "#F0FDF4", color: "#15803D" },
              { label: "Avg. per Expense", value: expenses.length ? `₹${formatAmount(totalAmount / expenses.length)}` : "—", icon: "📊", bg: "#FFF7ED", color: "#C2410C" },
            ].map(card => (
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 220, gap: 14 }}>
            ...
            <p style={{ color: "#94A3B8", fontSize: 14, margin: 0 }}>Loading expenses…</p>
          </div>
          ...
          <div style={{ textAlign: "center", padding: "72px 24px", background: "#F8FAFC", borderRadius: 16, border: "1.5px dashed #CBD5E1" }}>
            ...
            <p style={{ fontSize: 14, color: "#94A3B8", margin: "0 0 20px" }}>
              {isFiltered ? "Try adjusting your filters." : "Start tracking by adding your first expense."}
            </p>
            {!isFiltered && (
            ...
            {isFiltered && (
              <button onClick={clearFilters} style={{ background: "#F1F5F9", color: "#374151", border: "1.5px solid #E2E8F0", borderRadius: 9, padding: "10px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
                Clear Filters
              </button>
            )}
          </div>
            ...
            <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #E2E8F0", overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.05)", animation: "fadeIn 0.3s ease" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", background: "#F8FAFC", borderBottom: "1.5px solid #E2E8F0" }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: "#94A3B8", letterSpacing: "0.6px", textTransform: "uppercase" }}>
                  {isFiltered ? `${expenses.length} filtered results` : `Page ${page + 1} of ${totalPages || 1}`}
                </span>
                {isFiltered && (
                  <button onClick={clearFilters} style={{ fontSize: 12, color: "#6366F1", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>
                    ✕ Clear filters
                  </button>
                )}
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "1.5px solid #E2E8F0" }}>
                    {["#", "Title", "Amount", "Category", "Payment", "Date"].map((h, i) => (
                      <th key={h} style={{ padding: "12px 16px", textAlign: i === 2 ? "right" : "left", fontSize: 11, fontWeight: 700, color: "#94A3B8", letterSpacing: "1px", textTransform: "uppercase", whiteSpace: "nowrap" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    return (
                      <tr key={expense.id} className="exp-row" style={{ borderBottom: index < expenses.length - 1 ? "1px solid #F1F5F9" : "none" }}>
                        ...
                        <td style={{ padding: "14px 16px" }}>
                          <p style={{ fontSize: 14, fontWeight: 600, color: "#0F172A", margin: "0 0 2px" }}>{expense.title}</p>
                          {expense.notes && <p style={{ fontSize: 12, color: "#94A3B8", margin: 0 }}>{expense.notes}</p>}
                        </td>
                        ...
                        <td style={{ padding: "14px 16px" }}>
                          {expense.paymentMethod ? (
                            <span style={{ background: method.bg, color: method.text, borderRadius: 7, padding: "4px 10px", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>
                              {method.label}
                            </span>
                          ) : <span style={{ color: "#CBD5E1", fontSize: 13 }}>—</span>}
                        </td>
            ...
            {!isFiltered && totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 24 }}>
                <button className="page-btn" onClick={() => setPage(p => p - 1)} disabled={page === 0}
                  style={{ padding: "8px 14px", background: "#F8FAFC", color: "#374151", border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  ← Prev
                </button>
                {Array.from({ length: totalPages }, (_, i) => (
                  <button key={i} className="page-btn" onClick={() => setPage(i)}
                    style={{ width: 36, height: 36, background: i === page ? "#6366F1" : "#F8FAFC", color: i === page ? "#fff" : "#374151", border: `1.5px solid ${i === page ? "#6366F1" : "#E2E8F0"}`, borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                    {i + 1}
                  </button>
                ))}
      ...
      {showModal && (
        <div onClick={e => { if (e.target === e.currentTarget) closeModal(); }}
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: "32px", width: "100%", maxWidth: 520, boxShadow: "0 24px 64px rgba(0,0,0,0.2)", animation: "slideIn 0.25s ease", fontFamily: "'Inter', sans-serif", maxHeight: "90vh", overflowY: "auto" }}>

            {/* Modal Header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
              <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                <div style={{ width: 44, height: 44, background: "#EEF2FF", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>💸</div>
                <div>
                  <h2 style={{ fontSize: 19, fontWeight: 800, color: "#0F172A", margin: "0 0 2px", letterSpacing: "-0.4px" }}>New Expense</h2>
                  <p style={{ fontSize: 13, color: "#94A3B8", margin: 0 }}>Record a new expense entry</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                ...
                <input type="text" name="title" value={formData.title} onChange={handleFormChange}
                  placeholder="e.g. Grocery shopping" autoFocus style={inputBase}
                  onFocus={e => e.target.style.borderColor = "#6366F1"}
                  onBlur={e => e.target.style.borderColor = "#E2E8F0"}
                />
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div>
                  ...
                  <input type="date" name="expenseDate" value={formData.expenseDate} onChange={handleFormChange}
                    style={inputBase}
                    onFocus={e => e.target.style.borderColor = "#6366F1"}
                    onBlur={e => e.target.style.borderColor = "#E2E8F0"}
                  />
